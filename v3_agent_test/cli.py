...
Usage:
    ...
    python -m v3_agent_test --task "Design a 2PC protocol with coordinator and 2 banks"

    # Run a benchmark task (full Phase 1-5 pipeline)
    python -m v3_agent_test --benchmark 1E
    python -m v3_agent_test --benchmark 1E 2M 5H

    # Run all benchmark tasks
    python -m v3_agent_test --benchmark-all

    # Phase 5 only: generate prompts from an already-verified workspace
    # (mirror of the /tla-prompt-gen skill)
    python -m v3_agent_test --prompt-gen-only agent_workspace/workspace/claude46_run_1/3E

    # Filter by difficulty and/or scenario
    python -m v3_agent_test --benchmark-all --difficulty E --parallel 3
    python -m v3_agent_test --benchmark-all --difficulty H --scenario 1 2
    python -m v3_agent_test --benchmark-all --scenario 3

    # Options
    python -m v3_agent_test --benchmark 1E --model gpt-5 --provider openai
    python -m v3_agent_test --benchmark 1E --provider openrouter --model minimax/minimax-m2.5
    python -m v3_agent_test --benchmark 1E --max-turns 15 --verbose
"""
...
from v3_agent_test.loop import AgentLoop
from v3_agent_test.prompts import SYSTEM_PROMPT, PROMPT_GEN_SYSTEM_PROMPT
from v3_agent_test.session import SessionRecord, estimate_run_cost, format_run_cost
from v3_agent_test.tool_client import ToolClient
from v3_agent_test.tools import TOOL_SCHEMAS
from v3_agent_test.workspace import Workspace
from v3_agent_test.pipeline.llm_client import LLMConfig
...
def _build_config(args: argparse.Namespace) -> LLMConfig:
    ...
    # reasoning_effort: default "high" for openai only; "" for others unless explicit
...
def _run_single(
    ...
    workspaces_dir: str = "v3_agent_test/results/default/workspaces",
) -> tuple[str, Workspace]:
...
def main():
    ...
    # LLM config
    parser.add_argument(
        "--provider", default="openai", choices=["openai", "anthropic", "openrouter"]
    )
    ...
    # Create experiment directory: v3_agent_test/results/<timestamp>/workspaces/
    experiment_ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    experiment_dir = Path("v3_agent_test/results") / experiment_ts
