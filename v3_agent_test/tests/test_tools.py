"""Unit tests for v3_agent_test/tools.py — workspace tool functions."""
...
from v3_agent_test.workspace import Workspace, RepairAttempt
...
def ws_with_ir(ws):
    ...
    from v3_agent_test.tools import write_file
...
class TestEditFile:
    def test_edit_success(self, ws):
        from v3_agent_test.tools import write_file, edit_file
    ...
    def test_edit_nonexistent_file(self, ws):
        from v3_agent_test.tools import edit_file
...
class TestListFiles:
    def test_empty_workspace(self, ws):
        from v3_agent_test.tools import list_files
        result = list_files(ws)
        assert result == "Workspace is empty."

    def test_workspace_with_files(self, ws):
        from v3_agent_test.tools import write_file, list_files
...
class TestValidateIR:
    def test_valid_ir(self, ws_with_ir):
        from v3_agent_test.tools import validate_ir
    ...
    def test_invalid_ir_missing_agents(self, ws):
        from v3_agent_test.tools import write_file, validate_ir
...
class TestCompileScaffold:
    def test_no_ir_file(self, ws):
        from v3_agent_test.tools import compile_scaffold
...
class TestVerifySpec:
    def test_no_ir_file(self, ws):
        from v3_agent_test.tools import verify_spec
    ...
    def test_invalid_ir_fails_validation(self, ws):
        from v3_agent_test.tools import write_file, compile_scaffold, verify_spec
    ...
    def test_pcal_syntax_error(self, ws_with_ir):
        from v3_agent_test.tools import compile_scaffold, verify_spec
...
class TestCircuitBreaker:
    ...
    def test_circuit_breaker_does_not_trigger_on_different_violations(self, ws_with_ir):
        from v3_agent_test.tools import verify_spec, compile_scaffold
        ws = ws_with_ir
...
class TestExtractStates:
    def test_extract_no_translated_tla(self, ws_with_ir):
        from v3_agent_test.tools import extract_states
        result = extract_states(ws_with_ir)
        assert "ERROR" in result
        assert "Protocol_translated.tla" in result

    def test_extract_no_ir(self, ws):
        from v3_agent_test.tools import write_file, extract_states
    ...
    def test_extract_success(self, ws_with_ir):
        ...
        from v3_agent_test.tools import compile_scaffold, edit_file, verify_spec, extract_states
...
class TestLoadBenchmark:
    def test_valid_task_id(self, ws):
        from v3_agent_test.tools import load_benchmark
...
class TestThink:
    def test_returns_ok(self, ws):
        from v3_agent_test.tools import think
