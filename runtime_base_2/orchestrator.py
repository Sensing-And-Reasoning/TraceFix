...
runtime_B), but receive NO PlusCal protocol steps — uses NullMonitor
...
Prompt loading priority:
  1. File-based: workspace/prompts/runtime_base_2/{agent_id}.md (if dir exists)
...
from runtime_B.agent_runner import AgentRunner, AgentConfig, AgentResult
from runtime_B.coord import CoordinationContext, COORD_TOOL_SCHEMAS
from runtime_B.orchestrator import RunResult, _COORD_FOOTER
from runtime_base_2.null_monitor import NullMonitor
from runtime_base_2.prompt_gen import generate_b2_prompt
...
class RuntimeBase2:
    ...
    async def run(self, timeout: float = 120.0) -> RunResult:
        ...
        live_server = None
        if self.live:
            from runtime_B.event_bus import EventBus
            from runtime_B.live_server import start_live_server
            event_bus = EventBus()
        ...
        prompts_dir = self.workspace / "prompts" / "runtime_base_2"
        ...
        if live_server:
            from runtime_B.live_server import stop_live_server
