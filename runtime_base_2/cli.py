"""CLI entry point: python -m runtime_base_2 run --task 10H [options]."""
...
def _print_cost_summary(result, model: str) -> None:
    ...
    from runtime_B.cost import estimate_cost, format_cost
    print("\n=== Token Usage ===")
...
def _cmd_run(args):
    ...
    from runtime_base_2.orchestrator import RuntimeBase2
    ...
    if args.output:
        results_dir = Path(args.output) / "runtime_base_2" / args.task / ts
    else:
        results_dir = rt.workspace / "results" / "runtime_base_2" / ts
    ...
    try:
        ...
        from runtime_B.result_saver import save_run_result
...
def _save_visualization(rt, result, output_dir: Path, *, open_browser: bool = False):
    ...
    from runtime_B.visualize import save_html
...
def main():
    parser = argparse.ArgumentParser(
        prog="runtime_base_2",
        description="Baseline 2: Coordination primitives without protocol — same tools as runtime_B, no PlusCal",
    )
