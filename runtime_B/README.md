...
python -m runtime_B run --task 10H --workspace agent_workspace/10H --verbose

# With real-time visualization (browser opens during execution)
python -m runtime_B run --task 10H --workspace agent_workspace/10H --live --verbose

# Suppress auto-open of HTML after run
python -m runtime_B run --task 10H --workspace agent_workspace/10H --no-open-html

# Sim-environment tasks with failure injection
python -m runtime_B run --task 12E --workspace agent_workspace/12E --difficulty 2 --seed 42
python -m runtime_B run --task 16M --workspace agent_workspace/16M --scenario 2 --tool-time 0.5
python -m runtime_B run --task 13H --workspace agent_workspace/13H --difficulty 3 --seed 123
```
...
```bash
python -m pytest runtime_B/tests/ -v   # 45 tests
```
