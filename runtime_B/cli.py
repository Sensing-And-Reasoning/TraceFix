"""CLI entry point: python -m runtime_B run --task 10M --workspace agent_workspace/10M [options]."""
...
def _cmd_run(args):
    ...
    from runtime_B.orchestrator import RuntimeB
    ...
    if args.output:
        results_dir = Path(args.output) / "runtime_B" / args.task / ts
    else:
        results_dir = rt.workspace / "results" / "runtime_B" / ts
