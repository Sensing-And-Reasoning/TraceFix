"""CLI entry point: python -m runtime_A {viz|run} <ir.json> [options]."""
...
from runtime_A.topology import build_topology, load_ir
...
def _cmd_viz(args):
    ...
    from runtime_A.visualize import save_html
...
def _load_ir_for_run(args):
    ...
    """
    if args.task:
        from runtime_A.loader import load_task
...
def _build_policy(args, ir, task_id: str | None = None):
    ...
    sim = None
    if args.llm:
        from runtime_A.llm_policy import LLMPolicy
...
def _load_tool_registry(
    ...
    *,
    ...
    try:
        ...
        # Zero delay for runtime_A — engine controls timing
        config = ToolConfig(min_delay=0, max_delay=0, fail_probability=0)

        # Auto-discover sim module (same pattern as runtime_B)
        sim = None
        try:
...
def _print_result(result, args):
    ...
    # Per-agent summary
    from runtime_A.result_saver import _aggregate_agents
...
def _cmd_run(args):
    ...
    if args.llm:
        ...
        if args.output:
            results_dir = Path(args.output) / "runtime_A" / (task_id or "unknown") / ts
        else:
            results_dir = workspace / "results" / "runtime_A" / ts
    ...
    try:
        if getattr(args, "live", False):
            ...
            from runtime_A.engine import run_ir
...
def _save_results(
    ...
    from runtime_A.result_saver import save_run_result
    from runtime_A.visualize import save_run_html
...
async def _cmd_run_live(args, ir, title, policy, sim=None):
    ...
    from runtime_A.event_bus import EventBus
    from runtime_A.live_server import start_live_server, stop_live_server
    from runtime_A.engine import run_protocol
