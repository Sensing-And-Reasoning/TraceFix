...
| `store.py` | LockStore, CounterStore, MessageStore — shared coordination primitives (reused by runtime_B) |
...
python -m runtime_A viz /path/to/ir.json -o output.html --title "My Protocol"

# Run with random policy (baseline, no LLM)
python -m runtime_A run /path/to/ir.json --seed 42 --timeout 5 -q

# Run with LLM policy
python -m runtime_A run --task agent_workspace/3E \
  --llm --model gpt-4.1-mini --verbose --timeout 10

# Real-time visualization in browser
python -m runtime_A run --task agent_workspace/3E --llm --live --port 8765

# Sim-environment tasks with failure injection
python -m runtime_A run --task agent_workspace/12H --difficulty 2 --llm
python -m runtime_A run --task agent_workspace/4M --scenario 5 --llm
python -m runtime_A run --task agent_workspace/5H --tool-time 2.0 --llm

# Custom output, quiet mode
python -m runtime_A run --task agent_workspace/3E --output /tmp/results/ \
...
```bash
python -m pytest runtime_A/tests/ -v
```
