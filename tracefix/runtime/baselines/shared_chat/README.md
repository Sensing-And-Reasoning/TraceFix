...
**Key design**: Uses the **Chat Adapter Pattern** — `ChatCoordinationContext` duck-types tracefix.runtime.monitoring's `CoordinationContext`, enabling direct reuse of `AgentRunner` without modification. This ensures experimental fairness (same LLM client, retry logic, concurrent execution).
...
```bash
# Basic run
python -m tracefix.runtime.baselines.shared_chat run --task 3E --workspace agent_workspace/3E --verbose

# With real-time visualization
python -m tracefix.runtime.baselines.shared_chat run --task 3E --workspace agent_workspace/3E --live

# Sim-environment tasks with failure injection
python -m tracefix.runtime.baselines.shared_chat run --task 12E --workspace agent_workspace/12E \
  --difficulty 2 --seed 42

# Custom output directory
python -m tracefix.runtime.baselines.shared_chat run --task 3E --workspace agent_workspace/3E \
...
```
tracefix.runtime.monitoring         = Structured coordination + TLA+ protocol + ProtocolMonitor
tracefix.runtime.baselines.null_monitor    = Structured coordination + NullMonitor (no protocol)
tracefix.runtime.baselines.shared_chat    = Natural language chat only (no primitives)  ← this runtime
```
...
| Aspect | tracefix.runtime.monitoring | tracefix.runtime.baselines.shared_chat |
|--------|-----------|----------------|
