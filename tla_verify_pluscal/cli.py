...
def _resolve_jar(args: argparse.Namespace) -> str:
    return (
        ...
        or str(Path(__file__).parent.parent / "lib" / "tla2tools.jar")
    )
...
def cmd_validate(args: argparse.Namespace) -> int:
    from v3_agent_test.pipeline.validator import validate_ir
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    from v3_agent_test.pipeline.tlc_runner import run_tlc
...
def cmd_extract_states(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "legacy", False):
        from v3_agent_test.pipeline.tla_parser import parse_translated_tla
        result = parse_translated_tla(tla_content, ir_data)
    else:
        from v3_agent_test.pipeline.pluscal_parser import parse_pluscal
    ...
    from v3_agent_test.pipeline.pluscal_parser import lint_adjacent_acquire_release
