...
Reuses runtime_B.agent_runner.AgentRunner directly via the Chat Adapter
...
from runtime_B.orchestrator import RunResult
from runtime_base_1.chat_coord import (
    SharedChat, ChatCoordinationContext, CHAT_TOOL_SCHEMAS, CHAT_FOOTER,
)
from runtime_base_1.prompt_gen import generate_b1_prompt
...
class RuntimeBase1:
    ...
    Uses runtime_B.agent_runner.AgentRunner with ChatCoordinationContext
    ...
    async def run(self, timeout: float = 120.0) -> RunResult:
        ...
        # 5. Load domain tools (matches runtime_B: zero delay)
        tool_registry = None
        self.sim = None
        try:
        ...
        # 6. Create agent runners (reuse runtime_B's AgentRunner directly)
