"""CLI entry point: python -m runtime_base_1 run --task 10H [options]."""
...
def _cmd_run(args):
    ...
    from runtime_base_1.orchestrator import RuntimeBase1
    ...
    if args.output:
        results_dir = Path(args.output) / "runtime_base_1" / args.task / ts
    else:
        results_dir = rt.workspace / "results" / "runtime_base_1" / ts
...
def _save_visualization(rt, result, output_dir: Path, *, open_browser: bool = False):
    ...
    # Use star topology (group_chat hub) instead of protocol IR — runtime_base_1
